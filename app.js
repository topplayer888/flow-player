...
var area=document.getElementById("dc-form-result-area");
var btn=document.querySelector("#chat-form-dachuan .chat-form-submit");
if(area){area.style.display="block";area.textContent="生成中..."}
if(btn){btn.disabled=true;btn.textContent="生成中..."}
fetch(apiConfig.endpoint,{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+apiConfig.apikey},body:JSON.stringify({model:apiConfig.model,messages:[{role:"system",content:agents["1-1"].systemPrompt},{role:"user",content:prompt}],temperature:.7,max_tokens:5000})}).then(function(r){return r.json()}).then(function(data){
if(data.error){if(area)area.textContent="API 错误："+data.error.message;return}
if(!data.choices||!data.choices[0]||!data.choices[0].message){if(area)area.textContent="API 返回格式异常";return}
if(area)area.textContent=data.choices[0].message.content;
}).catch(function(e){if(area)area.textContent="网络请求失败："+e.message}).finally(function(){if(btn){btn.disabled=false;btn.textContent="🌊 生成电商脚本"}});
}
