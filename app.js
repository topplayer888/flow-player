var sectionModes=[-1,-1,-1];var sectionSavedKey=["","",""];var historyStack=[];try{var saved=localStorage.getItem("fp_history");if(saved){historyStack=JSON.parse(saved)}}catch(e){}try{var saved=localStorage.getItem("fp_history");if(saved){historyStack=JSON.parse(saved)}}catch(e){}
...
chatOpen=true;chatMessages=[];if(!savedMsgs)if(!savedMsgs)addHistory(section,mode);if(chatKey==='0-3'||chatKey==='2-0'){document.getElementById('chat-mode-tabs').style.display='none';switchChatMode('qa')}else if(agent.formOnly){document.getElementById('chat-mode-tabs').style.display='none';switchChatMode('form')}else{document.getElementById('chat-mode-tabs').style.display='';switchChatMode('qa')}document.querySelectorAll('.chat-mode-tab').forEach(function(t){t.classList.remove('active')});var firstTab=document.querySelector('.chat-mode-tab');if(firstTab)firstTab.classList.add('active');
