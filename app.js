var sectionModes=[-1,-1,-1];var sectionWasChatOpen=[false,false,false];
...
sectionWasChatOpen[currentSection]=chatOpen;currentSection=parseInt(e.dataset.section);currentMode=sectionModes[currentSection];closeMobileMenu();if(sectionWasChatOpen[currentSection]){renderRightModes();openChat(currentSection,currentMode)}else{if(chatOpen){closeChat()}renderContent();renderRightModes()}
});
});
...
var sparkTimer=0;
document.addEventListener("mousemove",function(e){mx=e.clientX;my=e.clientY;sparkTimer++;if(pmx>=0){var dx=mx-pmx,dy=my-pmy,dist=Math.sqrt(dx*dx+dy*dy);if(dist>8){spawnLightning(pmx,pmy,mx,my,8+Math.floor(dist/20),dist*.4)}if(sparkTimer%3==0){playElectric();spawnLightning(mx,my,null,null,6,12)}}pmx=mx;pmy=my});
document.addEventListener("mouseleave",function(){pmx=-1;pmy=-1});
document.addEventListener("touchmove",function(e){var t=e.touches[0];mx=t.clientX;my=t.clientY;sparkTimer++;if(pmx>=0){var dx=mx-pmx,dy=my-pmy,dist=Math.sqrt(dx*dx+dy*dy);if(dist>8){spawnLightning(pmx,pmy,mx,my,8+Math.floor(dist/20),dist*.4)}if(sparkTimer%3==0){playElectric();spawnLightning(mx,my,null,null,6,12)}}pmx=mx;pmy=my},{passive:true});
document.addEventListener("touchend",function(){pmx=-1;pmy=-1});
document.addEventListener("click",function(e){playClick();for(var i=0;i<6;i++){var ang=Math.random()*Math.PI*2;var r=30+Math.random()*50;spawnLightning(e.clientX,e.clientY,e.clientX+Math.cos(ang)*r,e.clientY+Math.sin(ang)*r,12,18)}for(var i=0;i<4;i++){var ang2=Math.random()*Math.PI*2;spawnLightning(e.clientX,e.clientY,null,null,8,20)}});
