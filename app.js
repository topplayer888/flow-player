...
{title:"素材类型",items:["3.1痛点共鸣型：适合破圈和新增人群。","3.2产品对比型：适合同品竞争和收割认知用户。","3.9机制收割型：适合促销、直播间引流和活动爆发。","也可根据产品自动扩展为情绪种草型、直播间引流型或复投迭代脚本。"]}];
...
var canvas=document.getElementById("bg-canvas"),ctx=canvas.getContext("2d"),particles=[],rainDrops=[],gridOffset=0,sparks=[],mx=-1,my=-1,pmx=-1,pmy=-1,isMobileViewport=window.matchMedia&&window.matchMedia("(max-width: 768px)").matches;
function resize(){isMobileViewport=window.matchMedia&&window.matchMedia("(max-width: 768px)").matches;canvas.width=window.innerWidth;canvas.height=window.innerHeight;var fc=document.getElementById("fx-canvas");if(fc){fc.width=window.innerWidth;fc.height=window.innerHeight;fxCtx=fc.getContext("2d")}initRain()}
...
function initRain(){rainDrops=[];if(isMobileViewport)return;var cols=Math.floor(canvas.width/28);for(var i=0;i<cols;i++){rainDrops.push({x:i*28+Math.random()*10,y:Math.random()*canvas.height,speed:1.2+Math.random()*2.5,len:6+Math.floor(Math.random()*16),bright:Math.random()>.85,phase:Math.random()*100})}}
...
for(var i=0;i<(isMobileViewport?0:150);i++)particles.push(new Particle);
initRain();
...
var sparkTimer=0;var rustSpots=[],debrisParticles=[];function triggerWastelandShake(){document.body.classList.remove("wasteland-shake");void document.body.offsetWidth;document.body.classList.add("wasteland-shake");setTimeout(function(){document.body.classList.remove("wasteland-shake")},220)}function spawnRust(x,y){if(isMobileViewport)return;rustSpots.push({x:x,y:y,r:3,life:45,max:45});if(rustSpots.length>50)rustSpots.shift()}function spawnImpact(x,y){if(isMobileViewport)return;for(var i=0;i<24;i++){var a=Math.random()*Math.PI*2,s=2+Math.random()*12;debrisParticles.push({x:x,y:y,vx:Math.cos(a)*s,vy:Math.sin(a)*s-4,life:20+Math.random()*35,size:1+Math.random()*5,alpha:.95})}for(var i=0;i<12;i++){var a2=Math.random()*Math.PI*2,r=5+Math.random()*25;debrisParticles.push({x:x+Math.cos(a2)*r,y:y+Math.sin(a2)*r,vx:Math.cos(a2)*.8,vy:Math.sin(a2)*.8-1.5,life:10+Math.random()*15,size:2+Math.random()*7,alpha:.7,isDust:1})}spawnRust(x,y)}function updateEffects(){for(var i=rustSpots.length-1;i>=0;i--){var s=rustSpots[i];s.life-=.8;s.r+=.2;if(s.life<=0)rustSpots.splice(i,1)}for(var i=debrisParticles.length-1;i>=0;i--){var d=debrisParticles[i];d.life--;d.x+=d.vx;d.y+=d.vy;d.vy+=.35;if(d.life<=0||d.y>canvas.height+100)debrisParticles.splice(i,1)}}function drawEffectsFx(){var ctx=fxCtx;function drawEffects(){for(var i=0;i<rustSpots.length;i++){var s=rustSpots[i],a=s.life/s.max,r=s.r;var g=ctx.createRadialGradient(s.x,s.y,0,s.x,s.y,r);g.addColorStop(0,"rgba(190,110,35,"+(a*.7)+")");g.addColorStop(.5,"rgba(150,80,20,"+(a*.45)+")");g.addColorStop(1,"rgba(100,50,10,0)");ctx.fillStyle=g;ctx.fillRect(s.x-r,s.y-r,r*2,r*2)}for(var i=0;i<debrisParticles.length;i++){var d=debrisParticles[i],a2=d.life/(d.isDust?25:55);ctx.fillStyle=d.isDust?"rgba(160,120,70,"+(a2*d.alpha)+")":(d.size>3?"rgba(130,75,25,":"rgba(180,110,50,")+(a2*d.alpha)+")";ctx.beginPath();ctx.arc(d.x,d.y,d.size,0,Math.PI*2);ctx.fill()}}drawEffects()}var fxCtx=null;function initFxCtx(){if(!fxCtx){var c=document.getElementById("fx-canvas");if(c){c.width=window.innerWidth;c.height=window.innerHeight;fxCtx=c.getContext("2d")}}}function effectsLoop(){if(isMobileViewport)return;initFxCtx();if(fxCtx){fxCtx.clearRect(0,0,fxCtx.canvas.width,fxCtx.canvas.height);if(themeWasteland){updateEffects();drawEffectsFx()}else{rustSpots=[];debrisParticles=[]}}requestAnimationFrame(effectsLoop)}effectsLoop();
document.addEventListener("mousemove",function(e){if(isMobileViewport)return;mx=e.clientX;my=e.clientY;sparkTimer++;if(themeWasteland){spawnRust(mx,my);if(sparkTimer%5==0)playMoveSound()}else{if(pmx>=0){var dx=mx-pmx,dy=my-pmy,dist=Math.sqrt(dx*dx+dy*dy);if(dist>8){spawnLightning(pmx,pmy,mx,my,8+Math.floor(dist/20),dist*.4)}if(sparkTimer%3==0){playMoveSound();spawnLightning(mx,my,null,null,6,12)}}}pmx=mx;pmy=my});
document.addEventListener("mouseleave",function(){pmx=-1;pmy=-1;if(themeWasteland)rustSpots=[]});
document.addEventListener("touchmove",function(e){if(isMobileViewport)return;var t=e.touches[0];mx=t.clientX;my=t.clientY;sparkTimer++;if(themeWasteland){spawnRust(mx,my);if(sparkTimer%5==0)playMoveSound()}else{if(pmx>=0){var dx=mx-pmx,dy=my-pmy,dist=Math.sqrt(dx*dx+dy*dy);if(dist>8){spawnLightning(pmx,pmy,mx,my,8+Math.floor(dist/20),dist*.4)}if(sparkTimer%3==0){playMoveSound();spawnLightning(mx,my,null,null,6,12)}}}pmx=mx;pmy=my},{passive:true});
document.addEventListener("touchend",function(){pmx=-1;pmy=-1;if(themeWasteland)rustSpots=[]});
document.addEventListener("click",function(e){playClickSound();if(isMobileViewport)return;if(themeWasteland){spawnImpact(e.clientX,e.clientY);triggerWastelandShake()}else{for(var i=0;i<6;i++){var ang=Math.random()*Math.PI*2;var r=30+Math.random()*50;spawnLightning(e.clientX,e.clientY,e.clientX+Math.cos(ang)*r,e.clientY+Math.sin(ang)*r,12,18)}for(var i=0;i<4;i++){var ang2=Math.random()*Math.PI*2;spawnLightning(e.clientX,e.clientY,null,null,8,20)}}});

!function(){if(isMobileViewport)return;var themes=["cool","warm","purple"];var chars=["01","0123456789ABCDEF",">_$#&@*%!","FLOW.01 FLOW.02 RANK.03","0xDEAD 0xBEEF 0xCAFE","{flow:player,rank:01}"];for(var i=0;i<12;i++){var d=document.createElement("div");d.className="data-stream "+themes[i%3];d.style.left=3+8*i+"%";d.style.setProperty("--dur",(7+7*Math.random())+"s");d.style.setProperty("--delay",(10*Math.random())+"s");d.style.animationDuration=(7+7*Math.random())+"s";d.style.animationDelay=(10*Math.random())+"s";var ch=chars[i%chars.length];var t="";for(var j=0;j<40;j++){t+=ch[Math.floor(Math.random()*ch.length)];if(j%8==7)t+="  "}d.textContent=t;document.body.appendChild(d)}}();
...
return fetch(apiConfig.endpoint,{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+apiConfig.apikey},body:JSON.stringify({model:apiConfig.model,messages:messages,temperature:.2,max_tokens:3500})}).then(function(r){return r.json()}).then(function(data){
if(data.error){throw new Error(data.error.message||"API 返回错误")}
if(!data.choices||!data.choices[0]||!data.choices[0].message){throw new Error("API 返回格式异常")}
return data.choices[0].message.content||"";
});
...
function updateApiStatus(){var btn=document.querySelector(".sidebar-settings-btn");if(!btn)return;btn.classList.toggle("api-configured",!!apiConfig.apikey)}
