...
animate();
var soundMode=parseInt(localStorage.getItem("fp_soundMode")||"1");var soundEnabled=localStorage.getItem("fp_soundEnabled")!=="0";function playMoveSound(){if(!soundEnabled)return;if(soundMode===1)playElectric();else playWaterFlow()}function playClickSound(){if(!soundEnabled)return;if(soundMode===1)playClick();else playWaterDrop()}function updateSoundUI(){var s=document.getElementById("set-sound-mode");if(s)s.value=soundMode;var t=document.getElementById("set-sound-toggle");if(t)t.value=soundEnabled?"1":"0"}var currentTheme=localStorage.getItem("fp_theme")||"cyberpunk";function applyTheme(t){currentTheme=t;if(t==="wasteland"){document.body.classList.add("theme-wasteland")}else{document.body.classList.remove("theme-wasteland")}localStorage.setItem("fp_theme",t);var s=document.getElementById("set-theme-mode");if(s)s.value=t}function updateThemeUI(){var s=document.getElementById("set-theme-mode");if(s)s.value=currentTheme}function saveThemeSettings(){currentTheme=document.getElementById("set-theme-mode").value;applyTheme(currentTheme);document.getElementById("settings-overlay").classList.remove("open")}applyTheme(currentTheme);
