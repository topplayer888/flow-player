...
if(savedMsgs&&savedMsgs.length>0){chatMessages=savedMsgs;var el=document.getElementById("chat-messages");el.innerHTML="";savedMsgs.forEach(function(m){var d=document.createElement("div");d.className="chat-msg "+m.role;d.innerHTML="<div class=\"chat-avatar\">"+(m.role==="user"?"U":"A")+"</div><div class=\"chat-bubble\">"+m.content+"</div>";el.appendChild(d)})}else{addMessage("assistant",agent.opening)}
}
var chatMode="qa";
...
function closeChat(){
if(chatMessages.length>0&&historyStack.length>0){saveHistoryLocal()}
...
function addMessageHTML(role,html){saveHistoryLocal()
