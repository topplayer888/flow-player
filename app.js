...
if(chatKey==="0-3"){formPanel.style.display="none";rwPanel.style.display="flex";updateRewriteApiStatus()}else if(chatKey==="0-0"){formPanel.style.display="none";rwPanel.style.display="none";var xp=document.getElementById("chat-form-xuehui");if(xp)xp.style.display="flex";xuehuiUpdateStatus()}else if(chatKey==="1-2"){formPanel.style.display="none";rwPanel.style.display="none";var tj=document.getElementById("chat-form-tiejia");if(tj)tj.style.display="flex";tjUpdateStatus();document.getElementById("tj-step1").style.display="";document.getElementById("tj-step2").style.display="none";document.getElementById("tj-step3").style.display="none";document.getElementById("tj-step4").style.display="none";document.getElementById("tj-step5").style.display="none"}else{formPanel.style.display="flex";rwPanel.style.display="none";updateFormApiStatus()}
}else{
...
function loadConfigUI(){document.getElementById("cfg-endpoint").value=apiConfig.endpoint;document.getElementById("cfg-apikey").value=apiConfig.apikey;var sel=document.getElementById("set-model");var known=Object.keys(MODEL_ENDPOINTS);if(known.indexOf(apiConfig.model)>=0){sel.value=apiConfig.model;document.getElementById("set-model-custom").style.display="none"}else{sel.value="__custom__";document.getElementById("set-model-custom").style.display="";document.getElementById("set-model-custom").value=apiConfig.model}}
...
var MODEL_ENDPOINTS={"deepseek-chat":"https://api.deepseek.com/v1/chat/completions","deepseek-reasoner":"https://api.deepseek.com/v1/chat/completions","gpt-4o":"https://api.openai.com/v1/chat/completions","gpt-4o-mini":"https://api.openai.com/v1/chat/completions","gpt-4-turbo":"https://api.openai.com/v1/chat/completions","qwen-plus":"https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions","qwen-max":"https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions","glm-4":"https://open.bigmodel.cn/api/paas/v4/chat/completions","moonshot-v1-8k":"https://api.moonshot.cn/v1/chat/completions"};function onModelSelectChange(){var sel=document.getElementById("set-model");var ci=document.getElementById("set-model-custom");if(sel.value==="__custom__"){ci.style.display="";ci.focus()}else{ci.style.display="none";var ep=MODEL_ENDPOINTS[sel.value];if(ep){document.getElementById("set-endpoint").value=ep}}}function saveSettingsApi(){apiConfig.endpoint=normalizeEndpoint(document.getElementById("set-endpoint").value.trim());apiConfig.apikey=document.getElementById("set-apikey").value.trim();var sel=document.getElementById("set-model");apiConfig.model=sel.value==="__custom__"?document.getElementById("set-model-custom").value.trim()||"deepseek-chat":sel.value;localStorage.setItem("fp_endpoint",apiConfig.endpoint);localStorage.setItem("fp_apikey",apiConfig.apikey);localStorage.setItem("fp_model",apiConfig.model);document.getElementById("settings-overlay").classList.remove("open");updateApiStatus();updateFormApiStatus()}function clearSettingsApi(){document.getElementById("set-endpoint").value="";document.getElementById("set-apikey").value="";document.getElementById("set-model").value="deepseek-chat";document.getElementById("set-model-custom").style.display="none";document.getElementById("set-model-custom").value="";apiConfig.endpoint="https://api.deepseek.com/v1/chat/completions";apiConfig.apikey="";apiConfig.model="deepseek-chat";localStorage.removeItem("fp_endpoint");localStorage.removeItem("fp_apikey");localStorage.removeItem("fp_model");document.getElementById("settings-overlay").classList.remove("open");updateApiStatus();updateFormApiStatus()}
...
function tjPick(el,groupId,max){var chips=document.getElementById(groupId).querySelectorAll(".select-chip");var selected=document.getElementById(groupId).querySelectorAll(".select-chip.selected");if(el.classList.contains("selected")){el.classList.remove("selected");if(groupId==="tj-hooks"){tjCheckHooksAuto()}else{tjCheckStep1Auto()}return}if(selected.length>=max){selected[0].classList.remove("selected")}el.classList.add("selected");if(groupId==="tj-hooks"){tjCheckHooksAuto()}else{tjCheckStep1Auto()}}
function tjCheckStep1Auto(){var p=tjGetVal("tj-product-type");var a=tjGetVal("tj-audience");var s=document.getElementById("tj-selling-points").value.trim();var pp=document.getElementById("tj-pain-points").value.trim();var sc=tjGetVal("tj-scene");var pr=tjGetVal("tj-price");if(p&&a&&s&&pp&&sc&&pr){tjStep1()}}
function tjCheckHooksAuto(){var hooks=tjGetVals("tj-hooks");if(hooks.length>=1){document.getElementById("tj-step3").style.display="";document.getElementById("tj-step2").style.display="none"}}function tjGetVal(id){var c=document.getElementById(id).querySelector(".select-chip.selected");return c?c.dataset.val:""}
...
function tjReset(){var ir=document.getElementById("tj-iterate-result");if(ir)ir.style.display="none";document.getElementById("tj-step1").style.display="";document.getElementById("tj-step2").style.display="none";document.getElementById("tj-step3").style.display="none";document.getElementById("tj-step4").style.display="none";document.getElementById("tj-step5").style.display="none";document.getElementById("tj-selling-points").value="";document.getElementById("tj-pain-points").value="";document.getElementById("tj-hooks-result").innerHTML="";document.getElementById("tj-result").textContent="";document.querySelectorAll("#tj-product-type .select-chip.selected,#tj-audience .select-chip.selected,#tj-scene .select-chip.selected,#tj-price .select-chip.selected,#tj-hooks .select-chip.selected,#tj-persona .select-chip.selected,#tj-tone .select-chip.selected").forEach(function(c){c.classList.remove("selected")})}