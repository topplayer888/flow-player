...
function drawRain(){ctx.font="13px Courier New,monospace";for(var r=0;r<rainDrops.length;r++){var d=rainDrops[r];d.phase+=d.speed*.15;for(var c=0;c<d.len;c++){var cy=d.y-c*20;if(cy<0||cy>canvas.height)continue;var chIdx=Math.floor((d.phase+c*3)%rainChars.length);var ch=rainChars[chIdx];var alpha=c==0?.9:c==1?.6:Math.max(0,(.4-c*.04));if(d.bright)alpha*=1.4;var hue=c==0?120:c<3?180:Math.random()>.5?190:35;ctx.fillStyle="hsla("+hue+",80%,"+(50+c*3)+"%,"+alpha+")";if(c==0){ctx.shadowColor="hsla("+hue+",100%,70%,"+alpha+")";ctx.shadowBlur=8}ctx.fillText(ch,d.x,cy);ctx.shadowBlur=0}d.y+=d.speed;if(d.y-d.len*20>canvas.height){d.y=-d.len*20+Math.random()*40;d.speed=1.2+Math.random()*2.5;d.bright=Math.random()>.85}}}
...
animate();
var soundMode=parseInt(localStorage.getItem("fp_soundMode")||"1");var soundEnabled=localStorage.getItem("fp_soundEnabled")!=="0";function playMoveSound(){if(!soundEnabled)return;if(soundMode===1)playElectric();else playWaterFlow()}function playClickSound(){if(!soundEnabled)return;if(soundMode===1)playClick();else playWaterDrop()}function toggleSound(){var t=document.getElementById("sound-toggle");if(!t)return;soundEnabled=!soundEnabled;if(soundEnabled){t.classList.add("on")}else{t.classList.remove("on")}}function saveSoundSettings(){soundMode=parseInt(document.getElementById("set-sound-mode").value);soundEnabled=document.getElementById("sound-toggle").classList.contains("on");localStorage.setItem("fp_soundMode",soundMode);localStorage.setItem("fp_soundEnabled",soundEnabled?"1":"0");document.getElementById("settings-overlay").classList.remove("open")}function updateSoundUI(){var sel=document.getElementById("set-sound-mode");if(sel)sel.value=soundMode;var tog=document.getElementById("sound-toggle");if(tog){if(soundEnabled){tog.classList.add("on")}else{tog.classList.remove("on")}}}
var audioCtx=null;function initAudio(){if(!audioCtx){audioCtx=new(window.AudioContext||window.webkitAudioContext)()}}
function playElectric(){if(!audioCtx||!soundEnabled)return;var dur=.06,ctx=audioCtx,rate=ctx.sampleRate,length=Math.floor(rate*dur),buf=ctx.createBuffer(1,length,rate),data=buf.getChannelData(0);for(var i=0;i<length;i++){data[i]=(Math.random()*2-1)*Math.exp(-i/(rate*.008))}var src=ctx.createBufferSource();src.buffer=buf;var bp=ctx.createBiquadFilter();bp.type="bandpass";bp.frequency.value=2000+Math.random()*3000;bp.Q.value=.5;var gain=ctx.createGain();gain.gain.setValueAtTime(.02,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(.001,ctx.currentTime+dur);src.connect(bp);bp.connect(gain);gain.connect(ctx.destination);src.start();src.stop(ctx.currentTime+dur)}
function playClick(){if(!audioCtx||!soundEnabled)return;var dur=.15,ctx=audioCtx,rate=ctx.sampleRate,length=Math.floor(rate*dur),buf=ctx.createBuffer(1,length,rate),data=buf.getChannelData(0);for(var i=0;i<length;i++){data[i]=(Math.random()*2-1)*Math.exp(-i/(rate*.02))}var src=ctx.createBufferSource();src.buffer=buf;var lp=ctx.createBiquadFilter();lp.type="lowpass";lp.frequency.setValueAtTime(800,ctx.currentTime);lp.frequency.exponentialRampToValueAtTime(80,ctx.currentTime+dur);var gain=ctx.createGain();gain.gain.setValueAtTime(.08,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(.001,ctx.currentTime+dur);src.connect(lp);lp.connect(gain);gain.connect(ctx.destination);src.start();src.stop(ctx.currentTime+dur)}
function playWaterFlow(){if(!audioCtx||!soundEnabled)return;var dur=.1,ctx=audioCtx,rate=ctx.sampleRate,length=Math.floor(rate*dur),buf=ctx.createBuffer(1,length,rate),data=buf.getChannelData(0);for(var i=0;i<length;i++){data[i]=(Math.random()*2-1)*Math.exp(-i/(rate*.02))*0.5+Math.sin(i*0.03)*(Math.random()*.3)}var src=ctx.createBufferSource();src.buffer=buf;var lp=ctx.createBiquadFilter();lp.type="lowpass";lp.frequency.value=600+Math.random()*400;lp.Q.value=.3;var gain=ctx.createGain();gain.gain.setValueAtTime(.015,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(.001,ctx.currentTime+dur);src.connect(lp);lp.connect(gain);gain.connect(ctx.destination);src.start();src.stop(ctx.currentTime+dur)}function playWaterDrop(){if(!audioCtx||!soundEnabled)return;var dur=.12,ctx=audioCtx,rate=ctx.sampleRate,length=Math.floor(rate*dur),buf=ctx.createBuffer(1,length,rate),data=buf.getChannelData(0);for(var i=0;i<length;i++){data[i]=(Math.random()*2-1)*Math.exp(-i/(rate*.01))*0.7}var src=ctx.createBufferSource();src.buffer=buf;var bp=ctx.createBiquadFilter();bp.type="bandpass";bp.frequency.setValueAtTime(1200,ctx.currentTime);bp.frequency.exponentialRampToValueAtTime(200,ctx.currentTime+dur);bp.Q.value=.8;var gain=ctx.createGain();gain.gain.setValueAtTime(.1,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(.001,ctx.currentTime+dur);src.connect(bp);bp.connect(gain);gain.connect(ctx.destination);src.start();src.stop(ctx.currentTime+dur)}
document.addEventListener("click",function(){initAudio()},{once:true});document.addEventListener("mousemove",function(){initAudio()},{once:true});
var sparkTimer=0;
document.addEventListener("mousemove",function(e){mx=e.clientX;my=e.clientY;sparkTimer++;if(pmx>=0){var dx=mx-pmx,dy=my-pmy,dist=Math.sqrt(dx*dx+dy*dy);if(dist>8){spawnLightning(pmx,pmy,mx,my,8+Math.floor(dist/20),dist*.4)}if(sparkTimer%3==0){playMoveSound();spawnLightning(mx,my,null,null,6,12)}}pmx=mx;pmy=my});
document.addEventListener("mouseleave",function(){pmx=-1;pmy=-1});
document.addEventListener("touchmove",function(e){var t=e.touches[0];mx=t.clientX;my=t.clientY;sparkTimer++;if(pmx>=0){var dx=mx-pmx,dy=my-pmy,dist=Math.sqrt(dx*dx+dy*dy);if(dist>8){spawnLightning(pmx,pmy,mx,my,8+Math.floor(dist/20),dist*.4)}if(sparkTimer%3==0){playMoveSound();spawnLightning(mx,my,null,null,6,12)}}pmx=mx;pmy=my},{passive:true});
document.addEventListener("touchend",function(){pmx=-1;pmy=-1});
document.addEventListener("click",function(e){playClickSound();for(var i=0;i<6;i++){var ang=Math.random()*Math.PI*2;var r=30+Math.random()*50;spawnLightning(e.clientX,e.clientY,e.clientX+Math.cos(ang)*r,e.clientY+Math.sin(ang)*r,12,18)}for(var i=0;i<4;i++){var ang2=Math.random()*Math.PI*2;spawnLightning(e.clientX,e.clientY,null,null,8,20)}});
