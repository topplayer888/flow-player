...
function getDurationRuleFromText(text,defaultDuration){
text=String(text||"");
var match=text.match(/(15-30秒|30秒以内|30-60秒|60-90秒|90-120秒|120秒以上|60秒以上|2分钟|两分钟)/);
return getStrictDurationRule(match?match[1]:defaultDuration);
}
...
function getMayuanDialogueSystemPrompt(base){if(chatKey==="1-1")return base+"\n\n# 大川内容体系对话式补充规则\n当用户要求生成电商脚本、短视频广告脚本、投放素材或口播文案时，先判断买点、身份视角、目标人群和素材类型，再生成结果。若用户没有明确指定时长，默认按30-60秒生成，并严格执行："+getStrictDurationRule("30-60秒")+"。若用户明确指定其它时长，必须按用户指定时长执行对应字数范围。"+getMergedShotScriptRule()+"\n\n输出不要使用Markdown表格、代码块或复杂嵌套列表。最后可以单独追加【纯口播文案】板块，方便用户复制拍摄。输出前必须自检【纯口播文案】是否符合时长字数要求，不符合就先重写。";if(!isMayuanChat())return base;if(chatKey==="1-3")return base+"\n\n# 马源2.0对话式补充规则\n当用户要求生成脚本、广告素材、测试计划、内容专项或裂变方案，并且没有明确指定时长时，默认按30-60秒脚本或首轮3-6条测试素材方向处理。默认脚本必须严格执行："+getStrictDurationRule("30-60秒")+"。若用户明确指定其它时长，必须按用户指定时长执行对应字数范围。输出不要使用Markdown表格、代码块或复杂嵌套列表。脚本输出按“脚本策略 / 完整分镜脚本 / 可替换要素 / 测试目的 / 裂变方向”分段。完整分镜脚本必须放到同一个板块里，不要拆成“口播逐字稿、拍摄画面、视觉高光”等多个板块；每个镜头按“时间段 / 画面动作 / 口播或字幕 / 视觉重点”连续输出，口播逐字稿放在对应镜头内，每句话单独成行。"+getMergedShotScriptRule()+"\n\n输出前必须自检【纯口播文案】是否符合时长字数要求，不符合就先重写。生成脚本后，在结尾追加一句：需要我帮你把这版内容篇幅加长吗？有任何修改意见请告诉我，我会帮你调整。";return base+"\n\n# 马源内容体系对话式补充规则\n当用户要求生成脚本、引流脚本、短视频文案或口播文案，并且没有明确指定时长时，优先按30-60秒生成，并严格执行："+getStrictDurationRule("30-60秒")+"。若用户明确指定30秒以内、30-60秒、60-90秒、90-120秒或120秒以上，以用户要求为准，并执行对应字数范围。\n\n# 输出排版要求\n使用清晰分段输出，不要使用Markdown表格、代码块或复杂嵌套列表。完整脚本建议按“脚本策略 / 完整分镜脚本 / 执行建议”分段；完整分镜脚本必须把时间段、画面/分镜、口播/字幕放在同一个镜头条目里，口播逐字稿放在对应镜头内，每句话单独成行，方便用户复制和拍摄。"+getMergedShotScriptRule()+"\n\n输出前必须自检【纯口播文案】是否符合时长字数要求，不符合就先重写。生成文案后，在结尾追加一句：需要我帮你把这版内容篇幅加长吗？有任何修改意见请告诉我，我会帮你调整。"}
...
var durationRule=getDurationRuleFromText(userMsg,isMayuanChat()||chatKey==="1-1"?"30-60秒":"");
