...
animate();
var soundMode=parseInt(localStorage.getItem("fp_soundMode")||"1");var soundEnabled=localStorage.getItem("fp_soundEnabled")!=="0";function playMoveSound(){if(!soundEnabled)return;if(soundMode===1)playElectric();else playWaterFlow()}function playClickSound(){if(!soundEnabled)return;if(soundMode===1)playClick();else playWaterDrop()}function setSoundOn(v){soundEnabled=v;var on=document.getElementById("sound-btn-on");var off=document.getElementById("sound-btn-off");if(on&&off){if(v){on.classList.add("active");off.classList.remove("active")}else{off.classList.add("active");on.classList.remove("active")}}}function saveSoundSettings(){soundMode=parseInt(document.getElementById("set-sound-mode").value);// soundEnabled already set by setSoundOn;localStorage.setItem("fp_soundMode",soundMode);localStorage.setItem("fp_soundEnabled",soundEnabled?"1":"0");document.getElementById("settings-overlay").classList.remove("open")}function updateSoundUI(){var sel=document.getElementById("set-sound-mode");if(sel)sel.value=soundMode;setSoundOn(soundEnabled)}
