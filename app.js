...
document.addEventListener("click",function(){initAudio()},{once:true});document.addEventListener("mousemove",function(){initAudio()},{once:true});var waterNoiseBuffer=null,lastWaterFlowAt=0;function getWaterNoiseBuffer(ctx,dur){var rate=ctx.sampleRate,len=Math.floor(rate*dur);if(waterNoiseBuffer&&waterNoiseBuffer.length===len)return waterNoiseBuffer;var buf=ctx.createBuffer(1,len,rate),data=buf.getChannelData(0),last=0;for(var i=0;i<len;i++){var white=Math.random()*2-1;last=last*.72+white*.28;data[i]=last*.75+white*.25}waterNoiseBuffer=buf;return buf}function playWaterFlow(){if(!audioCtx||!soundEnabled)return;var ctx=audioCtx,now=ctx.currentTime;if(now-lastWaterFlowAt<.075)return;lastWaterFlowAt=now;var dur=.24+Math.random()*.05,src=ctx.createBufferSource(),main=ctx.createGain(),low=ctx.createBiquadFilter(),air=ctx.createBiquadFilter(),pan=ctx.createStereoPanner?ctx.createStereoPanner():null;src.buffer=getWaterNoiseBuffer(ctx,dur);src.playbackRate.value=.92+Math.random()*.18;low.type="bandpass";low.frequency.value=360+Math.random()*180;low.Q.value=.7;air.type="highpass";air.frequency.value=900+Math.random()*350;main.gain.setValueAtTime(.0001,now);main.gain.exponentialRampToValueAtTime(.018,now+.025);main.gain.exponentialRampToValueAtTime(.006,now+dur*.68);main.gain.exponentialRampToValueAtTime(.0001,now+dur);if(pan)pan.pan.value=(Math.random()-.5)*.35;src.connect(low);low.connect(air);air.connect(main);if(pan){main.connect(pan);pan.connect(ctx.destination)}else{main.connect(ctx.destination)}src.start(now);src.stop(now+dur)}function playWaterDrop(){if(!audioCtx||!soundEnabled)return;var ctx=audioCtx,now=ctx.currentTime,master=ctx.createGain();master.gain.setValueAtTime(.9,now);master.connect(ctx.destination);function tone(type,f1,f2,g,d){var osc=ctx.createOscillator(),gain=ctx.createGain();osc.type=type;osc.frequency.setValueAtTime(f1,now);osc.frequency.exponentialRampToValueAtTime(f2,now+d);gain.gain.setValueAtTime(.0001,now);gain.gain.exponentialRampToValueAtTime(g,now+.006);gain.gain.exponentialRampToValueAtTime(.0001,now+d);osc.connect(gain);gain.connect(master);osc.start(now);osc.stop(now+d)}tone("sine",1050+Math.random()*260,360+Math.random()*80,.09,.18);tone("triangle",520+Math.random()*90,190+Math.random()*45,.035,.28);var src=ctx.createBufferSource(),buf=getWaterNoiseBuffer(ctx,.08),hp=ctx.createBiquadFilter(),ng=ctx.createGain();src.buffer=buf;hp.type="highpass";hp.frequency.value=1800;ng.gain.setValueAtTime(.025,now);ng.gain.exponentialRampToValueAtTime(.0001,now+.06);src.connect(hp);hp.connect(ng);ng.connect(master);src.start(now);src.stop(now+.08)}
