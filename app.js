...
"视频时长："+duration+"\n"+
"补充要求："+extra+"\n\n"+
"输出要求：先做买点分析和身份视角判断，再生成脚本。每条脚本都要包含素材类型、身份视角、目标人群、开头0-3秒、痛点放大、产品解决、内容深度、转化收口、画面建议、口播文案、字幕重点和投放建议。若身份视角为用户视角，必须以真实体验、使用感受、场景代入和提前拥有的快感来写；若为商家视角，必须强化产品区别、真实使用场景、产品特写和效果对比；若为权威专业IP视角，必须强化专业身份、可信形象、精准痛点覆盖和专业讲解。不要写成产品说明书，要像真实抖音电商短视频。";
return prompt;
}
...
var prompt="请根据以下修改意见，重新优化上一版大川电商脚本。保留大川内容体系的方法论，只返回优化后的完整结果，必须保留或重新明确身份视角。输出时请把口播逐字稿统一放在最后的【纯口播文案】板块里；前面的生成结果不要重复出现【口播文案】。\n\n修改意见："+adjust+"\n\n上一版内容：\n"+dachuanLastResult;
...
dcCallFormApi([{role:"system",content:agents["1-1"].systemPrompt},{role:"user",content:dachuanLastPrompt+"\n\n请重新生成一版，身份视角、角度、开头钩子和表达方式要和上一版有明显差异；如果用户指定了身份视角，则严格保留该身份视角。口播逐字稿只放在最后的【纯口播文案】板块，前面的生成结果不要重复出现【口播文案】。"}],"重新生成中...");
}
