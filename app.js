...
options=options||{};
try{
var apiHost=new URL(url,location.href).host.toLowerCase();
var requestBody=JSON.parse(options.body||"{}");
if(apiHost==="api.openai.com"&&/^gpt-(?:5|6)(?:[.-]|$)/i.test(String(requestBody.model||""))&&requestBody.max_tokens!==undefined){
...
var sections=[{title:"爆款脚本创作",subtitle:"Viral Script Creator",accent:"爆款",desc:"四大内容体系，精准产出爆款短视频脚本",modes:[{name:"薛辉内容体系",desc:"薛辉方法论 · 短视频爆款脚本的创作框架",icon:"🔥"},{name:"看见内容体系",desc:"看见方法论 · 内容触达与转化的核心逻辑",icon:"👁️"},{name:"访谈式IP策划",desc:"IP访谈 · 经历挖掘与短视频脚本生成",icon:"🎤"},{name:"爆款仿写",desc:"爆款仿写 · 对标爆款文案的结构化仿写生成",icon:"✍️"}]},{title:"广告创意",subtitle:"Ad Creative Studio",accent:"创意",desc:"四大创意体系，打造高转化广告素材",modes:[{name:"马源内容体系",desc:"马源方法论 · 广告创意的结构化表达",icon:"🚀"},{name:"大川内容体系",desc:"大川方法论 · 用户心智与创意触点",icon:"🌊"},{name:"铁甲内容体系",desc:"铁甲方法论 · 硬核卖点的创意包装",icon:"🛡️"},{name:"马源2.0",desc:"马源2.0 · 内容专项与广告创意智能体",icon:"🧠"}]},{title:"直播策略",subtitle:"Live Stream Strategy",accent:"直播",desc:"两大直播方法论，掌控直播间流量引擎",modes:[{name:"江导IP直播方法论",desc:"江导体系 · 直播间人货场全链路策略",icon:"🎯"},{name:"Kyrie直播方法论",desc:"Kyrie体系 · 知识付费直播闭环与中控训练",icon:"📈"}]}],currentSection=0,currentMode=0;
...
每次回复结尾必须根据当前阶段给出一个具体下一步：信息不足时引导用户补充本轮真正缺少的信息；只完成分析、摘要或策略时，引导用户确认方向或选择继续生成脚本、专项方案、测试计划；已经生成脚本时，引导用户说明要调整的镜头、语气、时长、卖点或转化方式。禁止无条件询问是否加长篇幅。`,
...
function resize(){canvas.width=window.innerWidth;canvas.height=window.innerHeight;initRain()}
window.addEventListener("resize",resize);resize();
function drawGrid(){var s=50;ctx.strokeStyle="rgba(255,255,255,0.02)";ctx.lineWidth=.5;ctx.beginPath();for(var x=0;x<=canvas.width;x+=s){ctx.moveTo(x,0);ctx.lineTo(x,canvas.height)}for(var y=gridOffset%s-s;y<=canvas.height+s;y+=s){ctx.moveTo(0,y);ctx.lineTo(canvas.width,y)}ctx.stroke()}
function drawAccentLines(){var s=50,o=gridOffset%s;for(var y=-s+o;y<=canvas.height+s;y+=s*5){var a=.03+.02*Math.sin(y*.01+gridOffset*.003);ctx.strokeStyle="rgba(0,229,255,"+a+")";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(canvas.width,y);ctx.stroke()}for(var y=-s*2+o;y<=canvas.height+s*2;y+=s*7){var a2=.025+.015*Math.sin(y*.015+gridOffset*.005+1);ctx.strokeStyle="rgba(255,160,30,"+a2+")";ctx.lineWidth=.8;ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(canvas.width,y);ctx.stroke()}}
...
Particle.prototype.reset=function(){this.x=Math.random()*canvas.width;this.y=canvas.height+10;this.size=Math.random()*2.5+.5;this.speed=Math.random()*.8+.3;this.opacity=Math.random()*.35+.1;this.hue=Math.random()>.55?190:Math.random()>.4?270:Math.random()>.5?35:20;this.glow=Math.random()>.7}
Particle.prototype.update=function(){this.y-=this.speed;if(this.y<-10)this.reset()}
Particle.prototype.draw=function(){var c="hsla("+this.hue+",90%,60%,"+this.opacity+")";if(this.glow){ctx.shadowColor=c;ctx.shadowBlur=6}ctx.fillStyle=c;ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,2*Math.PI);ctx.fill();ctx.shadowBlur=0}
for(var i=0;i<150;i++)particles.push(new Particle);initRain();
...
LightningBolt.prototype.draw=function(){if(this.life<=0)return;var a=this.life/this.maxLife;var w=a*1.5+.5;ctx.lineWidth=w;ctx.lineCap="round";ctx.shadowBlur=20+25*a;ctx.shadowColor="rgba(80,180,255,"+a+")";ctx.strokeStyle="rgba(100,200,255,"+Math.min(a*1.2,1)+")";ctx.beginPath();for(var i=0;i<this.segments.length;i++){var s=this.segments[i];ctx.moveTo(s.x1,s.y1);ctx.lineTo(s.x2,s.y2)}ctx.stroke();ctx.lineWidth=w*.5;ctx.shadowBlur=8+12*a;ctx.shadowColor="rgba(180,220,255,"+a+")";ctx.strokeStyle="rgba(200,240,255,"+Math.min(a*1.1,1)+")";ctx.beginPath();for(var i=0;i<this.segments.length;i++){var s=this.segments[i];ctx.moveTo(s.x1,s.y1);ctx.lineTo(s.x2,s.y2)}ctx.stroke();ctx.lineWidth=w*.2;ctx.shadowBlur=3+6*a;ctx.shadowColor="rgba(255,255,255,"+a+")";ctx.strokeStyle="rgba(255,255,255,"+a+")";ctx.beginPath();for(var i=0;i<this.segments.length;i++){var s=this.segments[i];ctx.moveTo(s.x1,s.y1);ctx.lineTo(s.x2,s.y2)}ctx.stroke();ctx.shadowBlur=0;ctx.lineCap="butt"}
...
function drawRain(){ctx.font="13px Courier New,monospace";for(var r=0;r<rainDrops.length;r++){var d=rainDrops[r];d.phase+=d.speed*.15;for(var c=0;c<d.len;c++){var cy=d.y-c*20;if(cy<0||cy>canvas.height)continue;var ch=rainChars[Math.floor((d.phase+c*3)%rainChars.length)];var alpha=c==0?.9:c==1?.6:Math.max(0,(.4-c*.04));if(d.bright)alpha*=1.4;var hue=c==0?120:c<3?180:Math.random()>.5?190:35;ctx.fillStyle="hsla("+hue+",80%,"+(50+c*3)+"%,"+alpha+")";if(c==0){ctx.shadowColor="hsla("+hue+",100%,70%,"+alpha+")";ctx.shadowBlur=8}ctx.fillText(ch,d.x,cy);ctx.shadowBlur=0}d.y+=d.speed;if(d.y-d.len*20>canvas.height){d.y=-d.len*20+Math.random()*40;d.speed=1.2+Math.random()*2.5;d.bright=Math.random()>.85}}}
function drawGlowPulses(){var t=gridOffset*.02;for(var i=0;i<3;i++){var px=canvas.width*(.2+.6*((Math.sin(t+i*2.1)+1)/2));var py=canvas.height*(.2+.6*((Math.cos(t*.7+i*1.7)+1)/2));var hue=i==0?190:i==1?35:270;var a=.015+.01*Math.sin(t*3+i);var g=ctx.createRadialGradient(px,py,0,px,py,canvas.width*.25);g.addColorStop(0,"hsla("+hue+",80%,55%,"+a+")");g.addColorStop(1,"transparent");ctx.fillStyle=g;ctx.fillRect(px-canvas.width*.25,py-canvas.width*.25,canvas.width*.5,canvas.width*.5)}}
function animate(){ctx.clearRect(0,0,canvas.width,canvas.height);gridOffset+=.3;drawGlowPulses();drawGrid();drawAccentLines();drawRain();for(var si=sparks.length-1;si>=0;si--){sparks[si].update();if(sparks[si].life<=0)sparks.splice(si,1);else sparks[si].draw()}particles.forEach(function(p){p.update();p.draw()});requestAnimationFrame(animate)}animate();var soundMode=parseInt(localStorage.getItem("fp_soundMode")||"1");var soundEnabled=localStorage.getItem("fp_soundEnabled")==="1";function playMoveSound(){if(!soundEnabled)return;if(soundMode===1)playElectric();else playWaterFlow()}function playClickSound(){if(!soundEnabled)return;if(soundMode===1)playClick();else playWaterDrop()}function updateSoundUI(){var s=document.getElementById("set-sound-mode");if(s)s.value=soundMode;var t=document.getElementById("set-sound-toggle");if(t)t.value=soundEnabled?"1":"0"}var currentTheme=localStorage.getItem("fp_theme")||"enterprise";function applyTheme(t){currentTheme=(t==="light"||t==="enterprise")?t:"enterprise";document.body.classList.remove("theme-light","theme-enterprise");document.body.classList.add("theme-"+currentTheme);var li=document.querySelector(".logo-icon");if(li)li.textContent=currentTheme==="light"?"✨":"⚡";var nis=document.querySelectorAll(".nav-icon");if(nis[0])nis[0].textContent=currentTheme==="light"?"📝":"🎬";if(nis[1])nis[1].textContent=currentTheme==="light"?"💎":"💡";if(nis[2])nis[2].textContent=currentTheme==="light"?"📊":"📡";localStorage.setItem("fp_theme",currentTheme);var s=document.getElementById("set-theme-mode");if(s)s.value=currentTheme}function updateThemeUI(){var s=document.getElementById("set-theme-mode");if(s)s.value=currentTheme}function saveThemeSettings(){localStorage.setItem("fp_theme",currentTheme);document.getElementById("settings-overlay").classList.remove("open");console.log("主题已保存:",currentTheme)}applyTheme(currentTheme);function saveSoundSettings(){soundMode=parseInt(document.getElementById("set-sound-mode").value);soundEnabled=document.getElementById("set-sound-toggle").value==="1";localStorage.setItem("fp_soundMode",soundMode);localStorage.setItem("fp_soundEnabled",soundEnabled?"1":"0");document.getElementById("settings-overlay").classList.remove("open")}var audioCtx=null;function initAudio(){if(!audioCtx){audioCtx=new(window.AudioContext||window.webkitAudioContext)()}}
...
var sparkTimer=0;document.addEventListener("mousemove",function(e){mx=e.clientX;my=e.clientY;sparkTimer++;if(pmx>=0){var dx=mx-pmx,dy=my-pmy,dist=Math.sqrt(dx*dx+dy*dy);if(dist>8)spawnLightning(pmx,pmy,mx,my,8+Math.floor(dist/20),dist*.4);if(sparkTimer%3==0){playMoveSound();spawnLightning(mx,my,null,null,6,12)}}pmx=mx;pmy=my});document.addEventListener("mouseleave",function(){pmx=-1;pmy=-1});document.addEventListener("touchmove",function(e){var t=e.touches[0];mx=t.clientX;my=t.clientY;sparkTimer++;if(pmx>=0){var dx=mx-pmx,dy=my-pmy,dist=Math.sqrt(dx*dx+dy*dy);if(dist>8)spawnLightning(pmx,pmy,mx,my,8+Math.floor(dist/20),dist*.4);if(sparkTimer%3==0){playMoveSound();spawnLightning(mx,my,null,null,6,12)}}pmx=mx;pmy=my},{passive:true});document.addEventListener("touchend",function(){pmx=-1;pmy=-1});document.addEventListener("click",function(e){playClickSound();for(var i=0;i<6;i++){var ang=Math.random()*Math.PI*2;var r=30+Math.random()*50;spawnLightning(e.clientX,e.clientY,e.clientX+Math.cos(ang)*r,e.clientY+Math.sin(ang)*r,12,18)}for(var i=0;i<4;i++){var ang2=Math.random()*Math.PI*2;spawnLightning(e.clientX,e.clientY,null,null,8,20)}});!function(){var themes=["cool","warm","purple"];var chars=["01","0123456789ABCDEF",">_$#&@*%!","FLOW.01 FLOW.02 RANK.03","0xDEAD 0xBEEF 0xCAFE","{flow:player,rank:01}"];for(var i=0;i<12;i++){var d=document.createElement("div");d.className="data-stream "+themes[i%3];d.style.left=3+8*i+"%";d.style.setProperty("--dur",(7+7*Math.random())+"s");d.style.setProperty("--delay",(10*Math.random())+"s");d.style.animationDuration=(7+7*Math.random())+"s";d.style.animationDelay=(10*Math.random())+"s";var ch=chars[i%chars.length];var t="";for(var j=0;j<40;j++){t+=ch[Math.floor(Math.random()*ch.length)];if(j%8==7)t+="  "}d.textContent=t;document.body.appendChild(d)}}();
...
- 每次回复结尾必须根据当前阶段给出一个具体下一步。信息不足时引导用户补充缺少的信息；只完成分析、摘要或策略时，引导用户确认方向或选择后续产物；已经生成脚本时，引导用户指出要调整的镜头、语气、时长、卖点或转化方式。禁止无条件询问是否加长篇幅。`,
opening:`你好，我是「马源2.0 · 内容专项智能体」🧠
...
if(chatKey==="0-0")return ["帮我生成更多选题","结合热点再生成选题","帮我换一组开头","帮我强化内容深度","帮我扩写并增加细节","帮我优化标题"];
...
}).catch(function(e){dcRenderFormResult("网络请求失败："+e.message)}).finally(function(){dcSetFormLoading(false)});
}
...
var MODEL_ENDPOINTS={"deepseek-v4-flash":"https://api.deepseek.com/v1/chat/completions","deepseek-v4-pro":"https://api.deepseek.com/v1/chat/completions","gpt-4o":OPENAI_CHAT_ENDPOINT,"gpt-4o-mini":OPENAI_CHAT_ENDPOINT,"gpt-4-turbo":OPENAI_CHAT_ENDPOINT,"qwen-plus":"https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions","qwen-max":"https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions","glm-4":"https://open.bigmodel.cn/api/paas/v4/chat/completions","moonshot-v1-8k":"https://api.moonshot.cn/v1/chat/completions"};
["gpt-6-astra","gpt-5.6-sol","gpt-5.6-terra","gpt-5.6-luna","gpt-5.5","gpt-5.4","gpt-5.4-mini","gpt-5.4-nano","gpt-5.2","gpt-5.1","gpt-5","gpt-5-mini","gpt-5-nano","gpt-4.1","gpt-4.1-mini","gpt-4.1-nano"].forEach(function(model){MODEL_ENDPOINTS[model]=OPENAI_CHAT_ENDPOINT});
function onModelSelectChange(){var sel=document.getElementById("set-model");var ci=document.getElementById("set-model-custom");if(!sel.value){ci.style.display="none";ci.value="";return}if(sel.value==="__custom__"){ci.style.display="";ci.focus()}else{ci.style.display="none";ci.value="";var ep=MODEL_ENDPOINTS[sel.value];if(ep){document.getElementById("set-endpoint").value=ep}}}function saveSettingsApi(){apiConfig.endpoint=normalizeEndpoint(document.getElementById("set-endpoint").value.trim());apiConfig.apikey=document.getElementById("set-apikey").value.trim();var sel=document.getElementById("set-model");var model=sel.value==="__custom__"?document.getElementById("set-model-custom").value.trim():sel.value;if(!model){alert("请先在模型选择栏选择你的模型");return}apiConfig.model=model;localStorage.setItem("fp_endpoint",apiConfig.endpoint);localStorage.setItem("fp_apikey",apiConfig.apikey);localStorage.setItem("fp_model",apiConfig.model);document.getElementById("settings-overlay").classList.remove("open");updateApiStatus();updateFormApiStatus()}function clearSettingsApi(){document.getElementById("set-endpoint").value="";document.getElementById("set-apikey").value="";document.getElementById("set-model").value="";document.getElementById("set-model-custom").style.display="none";document.getElementById("set-model-custom").value="";apiConfig.endpoint="https://api.deepseek.com/v1/chat/completions";apiConfig.apikey="";apiConfig.model="";localStorage.removeItem("fp_endpoint");localStorage.removeItem("fp_apikey");localStorage.removeItem("fp_model");document.getElementById("settings-overlay").classList.remove("open");updateApiStatus();updateFormApiStatus()}
...
function getMayuanContextualFollowupRule(){return "\n\n# 对话结尾规则（优先级高于前文固定结尾要求）\n禁止无条件询问是否加长篇幅。结尾只给一个与本轮实际完成阶段匹配的下一步：如果正在收集资料，就明确复述还需要用户补充的1-3项信息；如果只完成分析、摘要、策略或方案，就引导用户确认或调整本轮结论，并选择下一项要生成的产物；只有已经生成完整脚本或口播文案时，才引导用户说明要调整的镜头、语气、时长、核心卖点或转化方式。引导语必须承接本轮内容，不能使用与当前产物无关的万能话术。"}
var getMayuanDialogueSystemPromptBase=getMayuanDialogueSystemPrompt;
getMayuanDialogueSystemPrompt=function(base){
var prompt=getMayuanDialogueSystemPromptBase(base);
if(!isMayuanChat())return prompt;
prompt=prompt.split("\n\n生成脚本后，在结尾追加一句：需要我帮你把这版内容篇幅加长吗？有任何修改意见请告诉我，我会帮你调整。").join("");
prompt=prompt.split("\n\n生成文案后，在结尾追加一句：需要我帮你把这版内容篇幅加长吗？有任何修改意见请告诉我，我会帮你调整。").join("");
return prompt+getMayuanContextualFollowupRule();
};
function appendMayuanDialogueFollowup(content){
if(!isMayuanChat()||!content)return content;
var text=String(content).replace(/\s*需要我帮你把这版内容篇幅加长吗？\s*有任何修改意见请告诉我，我会帮你调整。?\s*$/," ").trim();
var hasGeneratedScript=/(?:完整分镜脚本|完整脚本\s*[：:]|纯口播文案|镜头\s*[一二三四五六七八九十\d]+\s*[｜|]|【🎙?口播】)/.test(text);
var needsUserInfo=/(?:信息不足|还缺|缺少|请.{0,12}(?:补充|回答|提供)|麻烦.{0,12}(?:补充|回答|提供)|按这个格式|先不.{0,8}(?:出|生成)脚本)/.test(text)&&!hasGeneratedScript;
var tail=text.slice(-320);
if(/(?:请|可以|直接|告诉我|回复|选择)[\s\S]{0,140}(?:补充|回答|提供|确认|选择|调整|生成|继续|发给我)/.test(tail))return text;
var followup;
if(needsUserInfo){
 followup="请根据上面的问题补充关键信息，我会接着完成下一步内容，不会跳过当前需要确认的部分。";
}else if(hasGeneratedScript){
 followup="你可以直接告诉我需要调整的镜头、语气、时长、核心卖点或转化方式，我会基于这版脚本继续修改。";
}else{
 followup="你可以告诉我这份分析中需要调整的部分，或者选择继续生成脚本、专项方案或测试计划，我会沿着当前结论继续完成。";
}
