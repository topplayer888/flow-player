...
var sections=[{title:"爆款脚本创作",subtitle:"Viral Script Creator",accent:"爆款",desc:"四大内容体系，精准产出爆款短视频脚本",modes:[{name:"薛辉内容体系",desc:"薛辉方法论 · 短视频爆款脚本的创作框架",icon:"??"},{name:"看见内容体系",desc:"看见方法论 · 内容触达与转化的核心逻辑",icon:"???"},{name:"访谈式IP策划",desc:"IP访谈 · 经历挖掘与短视频脚本生成",icon:"??"},{name:"爆款仿写",desc:"爆款仿写 · 对标爆款文案的结构化仿写生成",icon:"??"}]},{title:"广告创意",subtitle:"Ad Creative Studio",accent:"创意",desc:"四大创意体系，打造高转化广告素材",modes:[{name:"马源内容体系",desc:"马源方法论 · 广告创意的结构化表达",icon:"??"},{name:"大川内容体系",desc:"大川方法论 · 用户心智与创意触点",icon:"??"},{name:"铁甲内容体系",desc:"铁甲方法论 · 硬核卖点的创意包装",icon:"???"},{name:"马源2.0",desc:"马源2.0 · 内容专项与广告创意智能体",icon:"??"}]},{title:"直播策略",subtitle:"Live Stream Strategy",accent:"策略",desc:"两大直播方法论，掌控直播间流量引擎",modes:[{name:"江导IP直播方法论",desc:"江导体系 · 直播间人货场全链路策略",icon:"??"},{name:"Kyrie直播方法论",desc:"Kyrie体系 · 知识付费直播闭环与中控训练",icon:"??"}]}],currentSection=0,currentMode=0;
...
opening:"嗨！我是你的爆款仿写助手 ??\n\n请直接把你要仿写的短视频逐字文案发给我，我会：\n\n1?? 拆解分析——行业、人设、结构、钩子、情绪节奏\n2?? 让你选择仿写模式——原汁原味 or 自定义定位\n3?? 生成仿写文案\n\n直接粘贴文案，我们开始吧！",
...
function goHome(){if(chatOpen){closeChat()}document.body.classList.remove("admin-workspace");currentSection=0;currentMode=-1;document.querySelectorAll(".nav-item").forEach(function(n){n.classList.remove("active")});var nav=document.querySelector('.nav-item[data-section="0"]');if(nav)nav.classList.add("active");renderContent();renderRightModes();renderHistory()}
...
{id:"coach",key:"2-3",icon:"???",title:"老师训练",desc:"训练老师的四种能力和四种感觉。",tasks:[
...
{id:"interview",icon:"???",title:"访谈策划",desc:"把采访变成有目标、有追问、有剪辑方向的内容采集。",tasks:[
...
{id:"material",icon:"???",title:"素材转化",desc:"把访谈记录拆成可保存、可复用、可进入选题的素材资产。",tasks:[
...
var isMobileRuntime=window.matchMedia&&window.matchMedia("(max-width: 768px)").matches;
...
function initRain(){rainDrops=[];var gap=isMobileRuntime?42:28;var cols=Math.floor(canvas.width/gap);for(var i=0;i<cols;i++){rainDrops.push({x:i*gap+Math.random()*10,y:Math.random()*canvas.height,speed:1.2+Math.random()*2.5,len:6+Math.floor(Math.random()*(isMobileRuntime?10:16)),bright:Math.random()>.85,phase:Math.random()*100})}}
...
for(var i=0;i<(isMobileRuntime?55:150);i++)particles.push(new Particle);
initRain();
...
!function(){var themes=["cool","warm","purple"];var chars=["01","0123456789ABCDEF",">_$#&@*%!","FLOW.01 FLOW.02 RANK.03","0xDEAD 0xBEEF 0xCAFE","{flow:player,rank:01}"];var count=isMobileRuntime?5:12;for(var i=0;i<count;i++){var d=document.createElement("div");d.className="data-stream "+themes[i%3];d.style.left=(isMobileRuntime?6+18*i:3+8*i)+"%";d.style.setProperty("--dur",(7+7*Math.random())+"s");d.style.setProperty("--delay",(10*Math.random())+"s");d.style.animationDuration=(7+7*Math.random())+"s";d.style.animationDelay=(10*Math.random())+"s";var ch=chars[i%chars.length];var t="";for(var j=0;j<(isMobileRuntime?24:40);j++){t+=ch[Math.floor(Math.random()*ch.length)];if(j%8==7)t+="  "}d.textContent=t;document.body.appendChild(d)}}();
...
trackFeatureUsage({key:chatKey+(kyrieLaunch?":kyrie:"+kyrieLaunch.id+":"+pendingKyrieTaskIndex:(ipLaunch?":ip:"+ipLaunch.id+":"+pendingIPTaskIndex:"")),section:section,mode:mode,name:agent.name,workflowType:kyrieLaunch?"kyrie":(ipLaunch?"ip":""),moduleId:kyrieLaunch?kyrieLaunch.id:(ipLaunch?ipLaunch.id:""),taskTitle:kyrieTask?kyrieTask.title:(ipTask?ipTask.title:"")});
}
...
return "?? 尚未配置 API Key。<br><br><span class=\"api-config-hint\" onclick=\"openSettingsFromChat()\">? 点击此处配置 API</span>";
}
...
closeMobileMenu();
handleResize();
if(!(window.innerWidth<=768))renderHistory();
setTimeout(function(){
handleResize();
if(!(window.innerWidth<=768))window.dispatchEvent(new Event("resize"));
},window.innerWidth<=768?240:80);
}
