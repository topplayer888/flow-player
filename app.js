var sectionModes=[0,0,0];var sectionActive=[false,false,false];
...
if(agents[ak]){sectionActive[currentSection]=true;sectionModes[currentSection]=modeIdx;openChat(currentSection,modeIdx)}
...
currentSection=parseInt(e.dataset.section);currentMode=sectionModes[currentSection];closeMobileMenu();if(chatOpen){closeChat()}renderContent();renderRightModes();var ak=currentSection+"-"+currentMode;if(agents[ak]&&sectionActive[currentSection]){setTimeout(function(){openChat(currentSection,currentMode)},400)}
});
});
